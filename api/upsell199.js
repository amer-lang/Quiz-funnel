/* Sell Products AI — $199 "5 Done-For-You Video Ads" upsell checkout (VSL step).
   Vercel serverless function at /api/upsell199. Charges on OUR Stripe account
   ...
   The embedded fallback returns to ?upsold199=1&cs=<base>&upsell_cs=<cs_199>;
   dsResumeAfterUpsell199() verifies it and re-enters the launch sequence at
   the $49 image-ads step. Same SKU type as the $299 (video_ads_5) so
   fulfillment, metrics and the already-bought dedupe all treat them as one
   product — a buyer can never be charged for the pack twice. */

const READ_KEY = '448bd487135f59ca260b08fcb16d660e60b0953c54063d91cfeab0fe7e95362c';
const RETURN_URL = 'https://www.sellproducts.ai/?upsold199=1&cs={BASE}&upsell_cs={CHECKOUT_SESSION_ID}';
...
module.exports = async (req, res) => {
  ...
  try{
    ...
    /* ONE-CLICK: charge the payment method saved by the $20 unlock,
       off-session — no checkout UI at all. The idempotency key pins one $199
